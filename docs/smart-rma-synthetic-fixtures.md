...
P3-002 已按索引顺序对每个原始文本运行浏览器端解析器，并把 `expected_signals` 与缺失字段
作为确定性断言。解析器接受未知字段、保留未识别属性的“未知”语义，并在无法识别协议或关键
字段时返回显式 `unknown`，而不是猜测。输出合同和失败边界见
[SMART / RMA 浏览器解析器 v1](./smart-rma-parser-v1.md)。
