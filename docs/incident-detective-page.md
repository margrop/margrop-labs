...
8. 页面展示评分与 Attempt JSON，但不保存、不上传；AI 不参与评分；
9. 用户可以在评分后主动请求 AI 解释，失败时继续使用确定性 Findings；
10. 独立案例工坊可生成待人工审核的 Proposal，并下载 `publishable: false` 的本地审核包。

重新开始本局会同时清空证据、预算、假设、Attempt、评分和 AI 解释；修改已提交内容也会清除
旧分数与解释，避免结果与当前表单不一致。案例工坊状态与单局取证隔离。
