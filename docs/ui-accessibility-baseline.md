...
- Token Forge 数值、文本和多行文本使用原生控件，并将每条提示通过
  `aria-describedby` 关联；
- Token Forge 三档样例使用原生按钮，结果导航使用指向真实段落的原生锚点；
- 生成成功后结果标题获得一次程序化焦点；本地编辑不会重复移动焦点；
- 错误状态保留表单，并提供回到首字段和恢复 6K 本地样例两条键盘可用路径；
...
- Token Forge 的三档样例、字段、任务、验收项和导出卡在窄屏下逐级改为单列，结果导航
  在 600px 以下改为两列；
...
- Token Forge 的三档样例、三步引导、错误恢复、结果锚点与窄屏导航合同；
