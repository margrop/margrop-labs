...
接下来做 **P1-005**：基于已经验证的 Token Forge v1 计划实现 Markdown 和 GitHub Issue
文本导出。导出器只能接收结构化计划，不能包含仓库原文、文件路径、隐藏服务端指令或
Provider 元数据；文件名必须安全、结果可复制，并以合成 fixture 覆盖正常、脱敏、超长和
恶意文本边界。每次仍只处理一个任务。
