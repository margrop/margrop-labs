...
接下来做 **P3-003**：在浏览器内对用户粘贴的真实 SMART 文本实现序列号、WWN、主机名和
IP 完整脱敏，并为后续导出与 AI 边界提供只含允许字段的结构化投影。必须覆盖大小上限、
不同 smartctl 格式、重复标识、大小写与分隔符变体，以及 URL、日志、Analytics、导出和 AI
请求不含原文的失败测试。每次仍只处理一个任务；健康分类由 P3-004 单独实现。
