...
接下来只做 **P1-008**：为 P4-004 状态机接入服务端原子持久化、不可逆匿名用户键、固定
Provider/模型/价格表和所有 Provider 尝试的可信成本结算，再把 P1-004 AI 增强接到页面。
浏览器不得持有密钥、价格或结算字段；预算、限流、熔断、超时、Provider 与输出失败都必须
保留可导出的 P1-002 模板计划。Preview 验证后仍由人工触发 Production 发布，每次仍只
处理一个任务。
