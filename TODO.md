...
接下来做 **P3-002**：在浏览器端实现只读 SMART 解析器，消费 P3-001 的版本化合成语料，
结构化提取 smartctl 版本、协议、设备类别、健康摘要、关键 ATA 属性、NVMe 健康字段和错误
摘要；遇到缺失字段、未知厂商扩展或 SMART 不可用时返回显式未知，不上传或持久化原文。每次
仍只处理一个任务；序列号、WWN、主机名和 IP 的完整脱敏由 P3-003 单独实现。
