...
接下来做 **P1-004**：在 P0-006 AI Gateway 和 P0-007 脱敏边界上实现 AI 任务拆分。模型只能接收操作专属的最小允许字段和已脱敏仓库摘要；输出必须重新通过 Token Forge v1 Schema、预算/工时不变量、生产写操作禁令和相似任务去重，失败时完整降级到 P1-002 模板结果。每次仍只处理一个任务。
