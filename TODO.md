...
仓库所有者已要求跳过 Token 任务炼金炉的未完成需求，并按顺序完成 AI 面试工作台。当前
P5-001 至 P5-009 已闭环为只接受合成样例的 Alpha：

- P2、P3 不继续功能开发，只处理生产故障与安全问题；
- Token Forge 的 P1-014 NO-GO 和 P4-008 跳过记录保持不变，不重新打开功能任务；
- AI 面试工作台下一步只能先做真实输入边界的规划评审；
...
- [x] **P2-007 [L] 接入受约束的 AI 评分解释**
  - 产物：Explanation Input/Output v1 Schema、最小评分投影、Provider-neutral 服务端运行时、工作台按需请求与确定性降级。
  - 验收：AI 不接收 Attempt 自由文本、证据 Payload、内部答案或评分规则；输出只能引用输入中的 Finding、Evidence 和服务 ID；超时、限流、无效结构与网络失败不影响本地评分；请求正文不记录、不持久化。
- [x] **P2-008 [L] 提供 AI 案例 Proposal 与人工审核工坊**
  - 产物：Case Proposal 服务端端点、受限生成表单、结构化 Proposal 预览、本地 Human Review 门和审核包导出。
  - 验收：只接受合成主题、来源、预算和学习目标；所有生成结果先通过现有 DAG、预算、反证、只读与隐私验证；审核结果始终 `publishable: false`；AI 不可写仓库、部署或自动发布。

验收原则：用户可以在完全离线样例中完成一局；评分由确定性规则给出，AI 只负责解释与变体。

## P3：SMART / RMA 报告机 MVP（已恢复并完成）
...
- [x] **P3-003 [M] 实现序列号、WWN、主机名和 IP 脱敏**
  ...
  - 验收：覆盖序列号/WWN/主机名/IPv4/IPv6 的重复、大小写与分隔符变体；原文与自由文本不
    ...
    和日志副作用。
- [x] **P3-004 [M] 实现规则化健康指标与未知状态**
  - 产物：Health Assessment v1、Ruleset v1、正常/注意/危险/未知状态、冲突和建议动作界面。
  - 验收：7 份 fixture 精确匹配预期状态；PASSED 与异常证据分离；厂商扩展保持未知；规则无网络、
    存储或日志副作用；保修判断固定为 `not-determined`。
- [x] **P3-005 [M] 实现 AI 通俗解释**
  - 产物：AI Boundary/Input/Explanation v1、共享 Provider-neutral Runtime、显式 AI 操作与降级界面。
  - 验收：原文、脱敏预览、标识和隐私统计不进入请求；服务端固定 Provider/模型/Prompt/预算；超时、
    限流、无效结构和不可用均保留确定性结果；禁止虚构引用和保修结论。
- [x] **P3-006 [M] 生成中文摘要与英文 RMA Markdown**
  - 产物：Report Bundle v1、确定性中英文生成器、工作台本地下载和迁移说明。
  - 验收：输出可重复，不含原文、设备标识、Secret、时间戳或持久标识；英文材料只请求厂商复核，
    不声称必须保修；合成样例下载、移动端与基础无障碍 E2E 通过。

2026-07-29，仓库所有者明确批准恢复 P3-004 至 P3-006；恢复边界见 ADR-0009。
...
- [x] **P5-006 [L] 按顺序复用生产 AI 运行时**
  ...
  - 当前：已完成通用策略校验与注册表、三个同源端点、独立 `INTERVIEW_AI_POLICY` Durable Object、
    ...
    Preview 部署与 smoke 验收已随 P5-009 完成。
- [x] **P5-007 [L] 按顺序实现双角色三步工作台**
  ...
  - 当前：已接入双角色合成样例、岗位匹配/面试计划/记录与结论三步导航；记录编辑会重新
    通过引用合同并重算结论；恢复样例、安全摘要导出、AI 可选按钮和网关失败降级均已覆盖
    Chromium 回归。P5-009 已将页面开放为只接受合成样例的可索引 Alpha。
- [x] **P5-008 [L] 按顺序建立可靠性、公平性与隐私基准**
  - 产物：跨岗位合成语料、AI/网络/Schema/导出故障矩阵、受保护属性测试和 Chromium E2E。
  - 验收：引用、时长、评分与人工确认合同 100% 通过；五类 sink 无个人原文；所有 AI
    故障可降级；不存在自动录用或淘汰路径。
  - 当前：已增加云平台、前端产品、数据平台三个合成岗位族的双角色闭环基准；共享 Gateway
    和面试端点覆盖 Provider 5xx、限流、预算拒绝、无效 JSON、Schema 无效和输出上限；
    五类隐私 sink、受保护属性、自动决策失败关闭和三端点 Chromium 降级均有自动测试。
    详情见 [可靠性、公平性与隐私基线](./docs/interview-workbench-reliability-baseline.md)。
- [x] **P5-009 [M] 按顺序建立内容入口并执行 Alpha 出口评审**
  - 产物：博客 CTA、SEO/OG、无正文 Analytics、成本与隐私报告。
  - 验收：入口与三段转化可观察但不采集简历、JD、问题、记录或结论正文。

- [x] **P5-010 [M] 评审真实输入边界并建立威胁模型**
  - 产物：真实简历/JD 输入 ADR、数据流与信任边界、输入格式/大小/生命周期/删除/导出规则、
    XSS/注入/提示注入/恶意文件和敏感字段泄漏威胁模型。
  - 验收：只做规划决策，不实现真实输入；明确每类数据的本地处理、AI 最小投影和失败关闭；
    仓库所有者已批准继续执行 P5-011 至 P5-013。

- [x] **P5-011 [L] 实现本地简历与岗位说明接入内核**
  - 产物：版本化 Import/Input Schema、浏览器本地解析与规范化、大小/类型限制和合成 fixtures。
  - 验收：解析与 UI/网络/AI 分离；不持久化、不发送原文；有效/无效、XSS、注入、超限和
    五类敏感 sink 测试通过；合成模式保持可用。

- [x] **P5-012 [L] 实现真实输入本地工作台 UI**
  - 产物：经 ADR 批准的粘贴/导入、验证预览、显式清除、失败恢复和导出二次确认。
  - 验收：无需登录、AI 或持久化即可运行；320px、键盘、焦点、reduced motion 和基础
    无障碍通过；清除后不能恢复原文。

- [x] **P5-013 [L] 接入真实输入的 AI 最小投影**
  - 产物：三项操作的版本化 allowlist 投影、脱敏请求 Schema 和既有 AI 运行时集成。
...
- [ ] **P5-015 [M] 执行 Beta 出口评审**
  ...
  - 验收：`npm run validate` 和 Preview 关键路径通过；缺少真实质量或运营证据时保持 NO-GO；
    不在本任务自动执行 Production 发布。

## 推荐下一项任务

P4-006 暂缓，P4-008 按仓库所有者要求跳过，P1-014 仍记录为 **NO-GO**；仓库所有者已
明确批准绕过真实观察证据。P5-001 至 P5-009 已完成，AI 面试工作台达到**合成 Alpha GO**。
后 Alpha 顺序见 [Post-Alpha 路线图](./docs/interview-workbench-post-alpha-roadmap.md)。P5-010
至 P5-013 已完成；下一条最小任务仅为 **P5-014**：扩大隐私、公平性和对抗可靠性语料。
在 P5-015 评审前，真实招聘质量和 Production 使用仍保持 NO-GO。
