...
接下来做 **P2-003**：让 `/incident-detective/` 只加载 P2-002 的公开 `scenario.json`，
实现证据预算、前置解锁、证据卡片和按证据揭示的时间线，并在浏览器内生成可通过 v1
验证的 Attempt。内部答案和 canonical attempt 不得进入客户端；P2-004 前不显示伪评分。
每次仍只处理一个任务；真实 Analytics 写入继续等待 P4-003。
