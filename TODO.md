...
接下来做 **P3-004**：基于 Parse Result v1 和 Boundary Projection v1 实现确定性的健康、
注意、危险和未知分类。规则必须处理总体 PASSED 与关键计数非零的冲突，未知厂商属性不得
强行判故障，SMART 不可用或关键字段缺失必须显式为未知；每条结论需指向输入信号和规则，
不得调用 AI 或声称厂商必须保修。每次仍只处理一个任务。
