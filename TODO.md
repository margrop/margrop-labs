...
- [ ] **P1-006 [S] 建立文章入口与转化事件**
  - 产物：`/token-forge/` 正式页面、Token/Coding Plan 文章入口和最小转化事件合同。
  - 验收：无需登录、仓库和 AI 即可生成并导出一份模板计划；只记录打开、运行、导出和 GitHub 点击枚举，不记录表单、仓库 URL、计划或错误正文。
...
接下来做 **P1-006**：建立 Token Forge 正式页面与博客文章入口，把 P1-002 模板生成和
P1-005 本地导出接成无需登录、无需 AI 的完整合成路径；转化事件只允许稳定的打开、运行、
导出和 GitHub 点击枚举，禁止目标、仓库 URL、计划正文和错误正文。真实 Analytics 写入应
复用 P4-003 的隐私边界，未完成前只定义本地事件合同和测试。每次仍只处理一个任务。
