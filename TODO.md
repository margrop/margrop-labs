...
- [ ] **P1-008 [L] 接通受预算保护的生产 AI 规划**
  ...
  - 当前：代码、合成测试和 Wrangler dry-run 已完成；待配置 Preview Secrets、验证真实兼容接口，再由人工触发 Production 后关闭。
...
接下来只完成 **P1-008 激活**：配置 Preview Secrets，确认自定义端口的 DNS-only
前置条件，验证真实 Provider、限流、降级与浏览器无密钥，再由人工决定 Production。
Production smoke 通过后才关闭 P1-008 并进入 P1-009，每次仍只处理一个任务。
