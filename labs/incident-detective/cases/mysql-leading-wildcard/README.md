...
P2-002 的首个完整合成案例。公开 `scenario.json` 由页面加载；
`answer.internal.json` 只供仓库测试和案例审核参考，不得发送到客户端；
...
该答案文件不是评分规则：没有分数、权重或唯一理想顺序。P2-004 的
`score-rules.internal.json` 独立描述五个可审计维度，由页面在本地执行；它只引用结构化证据、
服务和安全动作，不包含或展示标准答案文本。
