...
5. 导出完整计划、GitHub Issue 草稿或 Provider-neutral Coding Agent 执行包。

当前 [`/token-forge/`](https://lab.margrop.net/token-forge/) Alpha 页面代码已开放完整的
本地模板、受限公开仓库摘要、显式 AI 增强、确定性质量评分、本地编辑和三种导出。仓库或
...
得分解释和排序原因；页面与三种导出共用同一份依赖安全顺序。仓库摘要只决定证据提示，
...
每次应用都会重新通过 Input / Plan v1、依赖先行、预算、质量与三种导出。PII 继续脱敏，
...
## Provider-neutral Coding Agent 执行包

[P1-011 执行包](../../docs/token-forge-agent-package.md) 把最终计划一一映射为依赖安全阶段，
为每个阶段给出允许/禁止上下文、Prompt、非目标、验收协议、交接模板和失败恢复。它要求
接收工具从实际工作区发现并记录仓库原生命令，不猜测文件路径、命令或外部系统。

执行包使用独立 v1 Schema 和固定 `token-forge-agent-package.md` 文件名，可复制或下载给
任意接受普通 Markdown 的 Coding Agent。生成完全本地，不绑定模型，不自动修改仓库，
不增加 AI 调用或 Token 成本。

## 正式页面与事件

[P1-006/P1-007 页面](../../docs/token-forge-page.md) 把模板输入、受限公开仓库证据和三种
导出接成浏览器内工作台。首页卡片链接到正式路由，结果区回到相关文章和 GitHub 源码。

[P1-012 首次体验](../../docs/token-forge-first-use.md) 增加 6K/24K/40K 三档一键模板样例、
三步引导、空/错/降级恢复和四段结果导航。24K 样例只读当前公开仓库，失败仍保留模板；
其他两档完全本地。所有样例无需登录和 AI。

## 端到端基准

[P1-013 基准与可靠性矩阵](../../docs/token-forge-benchmark.md) 使用 31 个完整计划场景、
15 个技术画像、8 份跨栈合成仓库快照和 35 个失败注入，持续验证 Schema、DAG、预算、
质量、降级、五类隐私 sink 与三种导出。机器报告由基准测试重新计算并防止过期。

真实 Chromium 关键路径覆盖 6K 本地生成与下载、GitHub 限流、AI 限流和 320px 键盘/
reduced-motion；浏览器测试已进入根目录统一 `npm run validate`，不访问真实仓库或模型。
...
生成完整计划、Issue 草稿和 Coding Agent 执行包。Alpha 页面已接入模板、公开仓库证据、
受控 AI 路径、质量评分、本地编辑与三种导出；真实 Preview 和 Production 调用已经验收。
