...
import {
  type IncidentDetectiveAiOperationDefinition,
  getIncidentDetectiveAiOperation,
  getIncidentDetectiveAiOperationByPath,
} from "./incident-detective-ai-registry";
import {
...
export const incidentDetectiveAiGatewayPolicy = Object.freeze({
...
const primaryAccountingTokens = 16_000;
const fallbackAccountingTokens = 32_000;

const createTrafficPolicy = (operation: string): Readonly<AiTrafficPolicy> =>
  Object.freeze(
    validateAiTrafficPolicy({
      schema_version: "1.0",
      lab_id: "incident-detective",
      operation,
      max_request_billable_tokens: fallbackAccountingTokens,
      max_request_cost_microusd: 1,
      daily_budgets: {
        actor_tokens: 64_000,
        lab_tokens: 640_000,
        site_tokens: 1_280_000,
        actor_cost_microusd: 4,
        lab_cost_microusd: 40,
        site_cost_microusd: 80,
        actor_requests: 4,
        lab_requests: 40,
        site_requests: 80,
      },
...
export const incidentDetectiveAiProductionTrafficPolicies = Object.freeze({
  explanation: createTrafficPolicy("incident-detective.explanation-v1"),
  caseProposal: createTrafficPolicy("incident-detective.case-proposal-v1"),
});

export const incidentDetectiveAiPreviewTrafficPolicies = Object.freeze({
  explanation: createAiPreviewPolicyLedger(
    incidentDetectiveAiProductionTrafficPolicies.explanation,
  ).policy,
  caseProposal: createAiPreviewPolicyLedger(
    incidentDetectiveAiProductionTrafficPolicies.caseProposal,
  ).policy,
});

type TrafficPolicyMode = "production" | "preview";

const resolveTrafficPolicyMode = (multiplier: string): TrafficPolicyMode => {
  if (multiplier === "1") return "production";
  if (multiplier === "100") return "preview";
  throw new Error("Invalid Incident Detective AI budget configuration.");
};

const createPolicyLedger = (
  operation: IncidentDetectiveAiOperationDefinition,
  mode: TrafficPolicyMode,
  snapshot?: unknown,
): TokenForgeAiPolicyLedger => {
  const policy = incidentDetectiveAiProductionTrafficPolicies[operation.key];
  return mode === "preview"
    ? createAiPreviewPolicyLedger(policy, snapshot)
    : new TokenForgeAiPolicyLedger(policy, snapshot);
};

export type IncidentDetectiveAiRuntimeEnvironment = {
...
export type IncidentDetectiveAiRuntimeOptions = {
  store: TokenForgePolicyStore;
  environment: IncidentDetectiveAiRuntimeEnvironment;
  fetch?: typeof fetch;
  now?: () => number;
};
...
const gatewayResponse = (
  body: AiGatewayResponse<JsonObject>,
  status?: number,
): Response =>
...
const parsePublicRequest = async (
  ...
  if (
    ...
    contentLength > incidentDetectiveAiGatewayPolicy.maxRequestBytes
  ) {
  ...
  const parsed = JSON.parse(
    await boundedText(
      request.body,
      incidentDetectiveAiGatewayPolicy.maxRequestBytes,
    ),
  ) as unknown;
  ...
  return { requestId: gatewayRequest.request_id, input: gatewayRequest.input };
};
...
const admit = async (
  ...
  store.mutate((snapshot) => {
    const ledger = createPolicyLedger(operation, mode, snapshot);
...
const makeGatewayOperation = (
  definition: IncidentDetectiveAiOperationDefinition,
  publicInput: JsonObject,
): AiGatewayOperation<JsonObject, JsonObject> => {
  let providerInput: JsonObject = publicInput;
  return {
    id: definition.operation,
    validateInput(candidate) {
      providerInput = definition.prepareInput(candidate);
      return providerInput;
    },
    validateOutput(candidate) {
      return definition.validateOutput(candidate, publicInput, providerInput);
    },
  };
};

export const handleIncidentDetectiveAiRequest = async (
  request: Request,
  options: IncidentDetectiveAiRuntimeOptions,
): Promise<Response> => {
  const url = new URL(request.url);
  const operation = getIncidentDetectiveAiOperationByPath(url.pathname);
  if (!operation)
  ...
  try {
    ...
    operation.prepareInput(parsed.input);
  } catch (error) {
  ...
  try {
    response = await executeAiGatewayRequest(
      ...
      {
        operation: makeGatewayOperation(operation, parsed.input),
        provider,
        policy: incidentDetectiveAiGatewayPolicy,
      },
    );
  } catch {
...
export const createMemoryIncidentDetectiveAiPolicyStore =
  (): TokenForgePolicyStore => {
...
export const getIncidentDetectiveAiOperationForRequest = (
  labId: string,
  operation: string,
): IncidentDetectiveAiOperationDefinition | undefined =>
  getIncidentDetectiveAiOperation(labId, operation);
